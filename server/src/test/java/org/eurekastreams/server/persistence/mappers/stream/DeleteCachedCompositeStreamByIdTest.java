...
public class DeleteCachedCompositeStreamByIdTest extends CachedMapperTest
{
    /**
     * System under test.
     */
    @Autowired
    DeleteCachedCompositeStreamById sut;
    ...
    public void setup()
    {
        ...
        List<Long> compositeStreamIdsList = new ArrayList<Long>();
        compositeStreamIdsList.add(COMPOSITE_STREAM_ID);

        // set list in the cache for the person.
        cache.setList(CacheKeys.COMPOSITE_STREAM_IDS_BY_PERSON_ID + PERSON_ID, compositeStreamIdsList);
    ...
    {
        sut.execute(PERSON_ID, COMPOSITE_STREAM_ID);
        
        //assert that cache is empty for compositeStream .
        assertEquals(0, 
                (cache.getList(CacheKeys.COMPOSITE_STREAM_IDS_BY_PERSON_ID + PERSON_ID)).size());
    }
