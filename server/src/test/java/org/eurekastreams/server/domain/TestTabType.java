...
public class TestTabType
{
    ...
    public void testEnum()
    {
        ...
        TabType[] enums = TabType.values();
        for (int i = 0; i < enums.length; i++)
        {
            assertEquals(message, enums[i], TabType.toEnum(enums[i].toString()));
            assertEquals(message, enums[i], TabType.valueOf(enums[i].name()));
        }

        // see what happens when you pass in something not parsable
        assertNull(message, TabType.toEnum("unrecognized"));
    }
}
