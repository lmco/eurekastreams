...
/**
 * This class provides the mechanism for retrieving the gadget metadata
 * contained with gadget definitions that are supplied.
 *
 */
public class GadgetMetaDataHttpFetcher implements GadgetMetaDataFetcher
{
    /**
     * Map of gadgetdefinitions that will have metadata retrieved for them.
     */
    private Map<String, GeneralGadgetDefinition> gadgetDefs;
    ...
    /**
     * Setter for the Current Application Context Base Url to be used for retrieving
     * gadget metadata from the container.
     * @param inCurrentAppContextBaseUrl - base url for the gadget metadata fetcher.
     */
     ...
     * @param inGadgetDefs - Map of gadget definitions with the string key as the 
     * gadget def url.
     * @return List of GadgetMetaData objects that contain the metadata for the Gadget
     * definitions passed in.
     * @throws Exception - error occurs on retrieving gadget metadata.
     */
    public List<GadgetMetaDataDTO> getGadgetsMetaData(
            final Map<String, GeneralGadgetDefinition> inGadgetDefs) throws Exception
    {
        List<GadgetMetaDataDTO> gadgetMetaData = new ArrayList<GadgetMetaDataDTO>();
        gadgetDefs = inGadgetDefs;
        if (gadgetDefs.size() > 0)
        {
            try
            {              
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        pipe(new StringReader(assembleMetaDataRequestContext()), writer);
                        writer.close();
                    }
                    catch (IOException e)
                    {
                ...
                gadgetMetaData = mapGadgetMetaDataJSONToObject(output.toString());
            }
            catch (Exception ex)
            {
    ...
    private List<GadgetMetaDataDTO> mapGadgetMetaDataJSONToObject(final String inGadgetsJSON)
    {
        ...
        for (int index = 0; index < gadgetsJSON.size(); index++)
        {
            ...
            currentGadget.setTitle(
                    gadgetJSON.containsKey("title") ? gadgetJSON.getString("title") : "");
    ...
    /**
     * This method assembles the context for the metadata request to the 
     * OpenSocial container.
     * @return String representation of the JSON Context.
     */
    private String assembleMetaDataRequestContext()
    {
     ...
     * @param reader - containing the reader content to be written.
     * @param writer - target writer.
     * @throws IOException on stream errors.
     */
    private static void pipe(final Reader reader, 
