...
public class RegisterUsageMetricExecution implements TaskHandlerExecutionStrategy<PrincipalActionContext>
{
     ...
     */
    private final DomainMapper<String, Long> personStreamScopeIdMapper;

    /**
     * Mapper to get a group stream scope id.
     */
    private final DomainMapper<String, Long> groupStreamScopeIdMapper;
    ...
    public Serializable execute(final TaskHandlerActionContext<PrincipalActionContext> inActionContext)
    {
                        ...
                        if (recipients.size() == 1)
                        {
                            ...
                            if (jsonObj.containsKey("type") && jsonObj.containsKey("name"))
                            {
                                ...
                                if ("PERSON".equals(jsonObj.getString("type")))
                                {
                                    ...
                                    logger.debug("Found person stream scope id " + streamScopeId
                                            + " from account id: " + uniqueKey);
                                }
        ...
        logger.trace("Registering metric for user: " + principal.getAccountId() + " StreamView:"
                + umdto.isStreamView() + " PageView:" + umdto.isPageView() + " MetricDetails: "
                + umdto.getMetricDetails());
