...
/**
 * Action to rebuild any cache related to an update of a Domain Group, designed
 * for async processing.
 */
public class DomainGroupCacheUpdaterAsyncExecution implements
								ExecutionStrategy<ActionContext>
{
    /**
     * Instance of the logger.
     */
    private Log log = LogFactory
            .getLog(DomainGroupCacheUpdaterAsyncExecution.class);
    ...
    /**
     * Mapper to add a private group id to the list of private group ids a user has access to view
     * activities for through a group/org coordinator role.
     */
     ...
     *            {@link AddPrivateGroupIdToCachedCoordinatorAccessList} mapper to add the target private group id
     *            to the list of group ids for each of the coordinators for the target group.
     * @param inCache - local cache instance.
     */
    ...
    /**
     * Perform the action, updating the coordinator cache list for a Domain
     * Group and rebuilding the security-scoped activity search strings for all
     * coordinators of a Domain Group.
     *
     ...
     */
    public final Serializable execute(final ActionContext inActionContext)
        	throws ExecutionException
    {
    	...
    	DomainGroupCacheUpdaterRequest request =
    		(DomainGroupCacheUpdaterRequest) inActionContext.getParams();
        ...
        log.info("Updating the cached list of coordinators for group #"
                + domainGroupId + " from the database");
        ...
        if (!domainGroup.isPublicGroup())
        {
            if (request.getIsUpdate())
            {
                log.info("Looping across the coordinators to update"
                        + " their private group access list to include the group #"
                        + domainGroupId);
                for (Long coordinatorPersonId : peopleIds)
                {
                    cache.addToSet(
                            CacheKeys.PRIVATE_GROUP_IDS_VIEWABLE_BY_PERSON_AS_COORDINATOR + coordinatorPersonId,
                            domainGroupId);
                }
            }
