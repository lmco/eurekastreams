...
/**
 * Delete an activity (and associated comments) from DB and update current user's CompositeStreams
 * (and "Everyone" CompositeStream) in cache so UI functions correctly without lag.
 * NOTE: This will NOT update all CompositeStreams in cache that may contain the 
 * activity.
 *
 */
...
{    
    ...
    /**
     * Constructor.
     * @param inActivityDAO Activity DAO.
     * @param inUserCompositeStreamIdsDAO CompositeStream Ids by user DAO.
     * @param inUserCompositeStreamDAO CompositeStreams by id DAO.
     * @param inBulkPeopleByAccountIdMapper People by account id DAO.
    ...
    /**
     * Delete an activity from DB and update current user's CompositeStreams
     * (and "Everyone" CompositeStream) in cache so UI functions correctly without lag.
     * @param inDeleteActivityRequest The DeleteActivityRequest object.
     * @return The list of comment ids associated with the activity. If the activity 
     * doesn't have any comments, an empty list will be returned. If the activity to be 
     * deleted is no longer present, null will be returned as "short-circuit" value.
     */
    ...
    public ActivityDTO execute(final DeleteActivityRequest inDeleteActivityRequest)
    {
        ...
        getEntityManager().createQuery("DELETE FROM Comment c WHERE c.target.id = :activityId")
            .setParameter("activityId", activityId).executeUpdate();
        
        //delete activity from currentUser's starred activity collections in DB.
        getEntityManager().createQuery("DELETE FROM StarredActivity where activityId=:activityId")
            .setParameter("activityId", activityId).executeUpdate();
        
        //delete activity from DB.
        getEntityManager().createQuery("DELETE FROM Activity WHERE id = :activityId")
            .setParameter("activityId", activityId).executeUpdate();
        ...
        removeActivityIdFromCompositeStreamListInCache(
                getDestinationCompositeStreamId(activity), activityId);
        
        //Get user's composite streams
        List<StreamFilter> compositeStreams = userCompositeStreamDAO.execute(
                userCompositeStreamIdsDAO.execute(userId));
     ...
     * @param inStreamView The StreamView
     * @param inStreamId The StreamId
    ...
    /**
     * Removes provided activityId from list of activities for the provided compositeStreamId in 
     * cache.
     * @param inCompositeStreamId CompositeStream id.
