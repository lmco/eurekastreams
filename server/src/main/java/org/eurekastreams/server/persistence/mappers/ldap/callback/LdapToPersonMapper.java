...
public class LdapToPersonMapper implements AttributesMapper 
{
	...
	/**
	 * Transformer to convert the Attributes to a display name suffix
	 * (optional).
	 */
	...
	public Object mapFromAttributes(final Attributes attrs)
		...
		try {
			log.debug("Found person: "
					+ attrs.get(fullNameAttrib).get().toString());
			...
			String email = (null != attrs.get(emailAttrib)) ? attrs
					.get(emailAttrib).get().toString() : "";

			String middleName = (null != attrs.get(middleNameAttrib)) ? attrs
					.get(middleNameAttrib).get().toString() : "";

			String companyName = (null != attrs.get(companyAttrib)) ? attrs
					.get(companyAttrib).get().toString() : "";

			String title = (null != attrs.get(titleAttrib)) ? attrs
					.get(titleAttrib).get().toString() : "";

			String[] splitCn = attrs.get(fullNameAttrib).get().toString()
					.split(", ");
			String preferredName = (splitCn.length > 1) ? splitCn[1]
					: firstName;

			person = new Person(accountId, firstName, middleName, lastName,
					preferredName);
			...
			if (additionalProperties != null) {
				...
				for (String property : additionalProperties) {
					...
					try {
						propertiesMap.put(property, attrs.get(property).get()
								.toString());
					} catch (Exception ex) {
						if (log.isInfoEnabled()) {
							log.info("Additional Property: " + property
									+ " not found for user " + accountId);
						}
					}
				}
	...
	public void setAdditionalProperties(
			final List<String> inAdditionalProperties) {
